...
- **Parallel UNIT_FIX**: When multiple independent modules fail, fix them in parallel
  - UNIT_FIX (different modules): parallel rtl-p4s-bugfix tasks, one per module
...
1.5. **Classify and Batch Parallel Fixes**:
   ...
   - **If failures are in DIFFERENT modules** (independent):
     → Launch parallel rtl-p4s-bugfix tasks, one per module
   ...
   - **If failures are in the SAME module** (dependent):
     → Sequential fix within a single rtl-p4s-bugfix task (fix all issues together)
...
4. **Functional verification step**: Run simulation and check results
   - eda-runner executes simulation:
     ...
     # Fallback: SIM=icarus / --sim iverilog (for 4-state X/Z simulation or verilator-unsupported SV constructs)
     ```
...
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb test via Bash CLI: make -C sim/{module} SIM=verilator TOPLEVEL={module} MODULE=test_{module}. Report pass/fail. On failure, capture waveform for debug.")
...
# ============================================================
# When rtl-p4s-bugfix is invoked after a FAIL in a Phase 5 sub-phase:
...
# → rtl-p4s-bugfix invoked (feedback_origin=5a-formal)
...
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb test for module_a: make -C sim/module_a SIM=verilator TOPLEVEL=module_a MODULE=test_module_a",
     run_in_background=true)
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb test for module_b: make -C sim/module_b SIM=verilator TOPLEVEL=module_b MODULE=test_module_b",
     run_in_background=true)
...
- Use the rtl-p5s-func-verify skill: `/rtl-agent-team:rtl-p5s-func-verify`
