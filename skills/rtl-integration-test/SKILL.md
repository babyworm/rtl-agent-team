...
- Only need single-module regression (use rtl-func-verify — Tier 3)
- Performance measurement (use rtl-perf-verify)
...
6. **Simulation:**
   ...
   - cocotb:
     ```bash
     make -C sim/top SIM={sim} TOPLEVEL={top} MODULE=test_{top}_integration
     ```
...
# cocotb path
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb integration test: make -C sim/top SIM=icarus TOPLEVEL={top} MODULE=test_{top}_integration. Report pass/fail per test.")
...
Integration test on 6-module pipeline:
  ...
  RTL fix applied via rtl-bugfix; retest all PASS;
...
- Use rtl-cdc-verify for detailed CDC analysis

**Tier transition rules:**
- Tier 3 PASS (rtl-func-verify) → Tier 4 eligible
- Tier 4 PASS (this skill) → Phase 5 final compliance eligible
- Tier 4 FAIL → isolate to module, fix via rtl-bugfix, retest Tier 2→3→4
</Advanced>
