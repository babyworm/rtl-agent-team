...
- Only unit-level SV tests needed (use rtl-p4s-unit-test — Tier 2)
- Integration/cross-module testing needed (use rtl-p5s-integration-test — Tier 4)
- Performance comparison needed (use rtl-p5s-perf-verify instead)
</Do_Not_Use_When>
...
2. **Pipelined TB Generation + Execution (per-module parallel)**:
   ...
   - As EACH module's TB completes → immediately launch eda-runner for that module:
     ```bash
     # Verilator (default — fast compiled simulation, good coverage support)
     make -C sim/{module} SIM=verilator TOPLEVEL={module} MODULE=test_{module}
     # Fallback: iverilog (for 4-state X/Z simulation, delay modeling, or verilator-unsupported SV constructs)
...
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb regression via Bash CLI: make -C sim/cabac_encoder SIM=verilator TOPLEVEL=cabac_encoder MODULE=test_cabac_encoder RANDOM_SEED=42. Report pass/fail per test and overall coverage.",
...
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb regression: make -C sim/transform SIM=verilator TOPLEVEL=transform MODULE=test_transform RANDOM_SEED=42.",
     run_in_background=true)
...
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run full multi-seed regression via Bash CLI: bash skills/rtl-regression-run/scripts/run_regression.sh --seeds '1 42 123 1337 65536' --sim verilator --parallel 4. Report per-seed pass/fail, capture .vcd on failure. Save results to sim/regression/seed_{seed}_results.json.",
     run_in_background=true)

# Option B: Manual per-seed launch (for fine-grained control)
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb regression: make -C sim/cabac_encoder SIM=verilator TOPLEVEL=cabac_encoder MODULE=test_cabac_encoder RANDOM_SEED=123.",
     run_in_background=true)
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb regression: make -C sim/cabac_encoder SIM=verilator TOPLEVEL=cabac_encoder MODULE=test_cabac_encoder RANDOM_SEED=1337.",
     run_in_background=true)
Task(subagent_type="rtl-agent-team:eda-runner",
     prompt="Run cocotb regression: make -C sim/cabac_encoder SIM=verilator TOPLEVEL=cabac_encoder MODULE=test_cabac_encoder RANDOM_SEED=65536.",
     run_in_background=true)
# → 5 seeds × N modules = up to 5N parallel sim tasks
...
- **Requirement traceability verdict FAIL with persistent test failures** → escalate to rtl-p4-implement for RTL fix before re-verification
</Escalation_And_Stop_Conditions>
