> **English version**: [TEST-GUIDE.md](./TEST-GUIDE.md)
...
| `test_run_conformance.py` | `run_conformance.py` | 스트림 탐색, MD5 계산, 디코딩 결과 구조, 프로파일/레벨 필터 |
...
| `agents/` 평면 구조 유지 | 중첩 `.md` 금지 — 중첩 파일은 유령 에이전트로 등록됨 |
| 스킬 SKILL.md 존재 | 97개 스킬 디렉토리마다 `SKILL.md` 존재 확인 |
| 스킬 이름-디렉토리 일치 | `skills/rtl-p4-implement/SKILL.md`의 `name: rtl-p4-implement` 일치 확인 |
| 스킬 description 예산 | 스킬당 160자 이하, 전체 15 KB 이하 — 전역 예산 초과 시 하네스가 조용히 절삭 |
| 스킬 자산 경로 | skill-relative 자산 경로 해석 가능 여부, 런타임에 없는 플러그인 내부 경로 참조 금지 |
...
| 파이프라인 규칙 커버리지 | CLAUDE.md에 선언된 모든 규칙이 rtl-orchestrate 본문과 SessionStart 주입 양쪽에 도달하는지 확인 |
...
| BUG-003 | `check_conventions.sh` | Rule 5가 첫 토큰만 검사해 `unique case (x)`를 인스턴스 `case`로 오판. 같은 패턴이 파라미터화된 인스턴스는 아예 미검출 | **FIXED** |
...
**Q: macOS에서 일부 테스트가 SKIP되는데요?**
A: `generate_config.sh`가 `BASH_VERSINFO[0] >= 4`로 자체 가드하는데 macOS는 bash 3.2를 탑재합니다. `tests/conftest.py::find_bash4`가 bash 4 이상을 탐지하고 없으면 해당 테스트를 SKIP합니다. `brew install bash`로 설치하면 실행되며, Linux CI에서는 항상 실행됩니다.
