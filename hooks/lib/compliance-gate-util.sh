...
compliance_preprocess() {
  ...
  # Freshness guard: compliance-state.json is (re)written/touched on every skill
  # activation by rtl-phase-state-bootstrap — on a phase change (new content) AND
  # on same-phase re-invocation (mtime refresh). A compliance-report.json older
  # than that marker therefore belongs to a prior phase OR a prior run of this
  # skill, and must NOT auto-satisfy the current phase's compliance-pass; leaving
  # it pending forces this run's own compliance check to produce a fresh report.
