...
50개 전문 AI 에이전트 + 37개 스킬 + 11개 레퍼런스 문서를 통해 6-Phase 설계 파이프라인(Research → Architecture → μArch → RTL → Verify → Design Note)을 자동화합니다.
...
전체 37개 스킬 목록은 `skills/` 디렉토리를 참조하세요.
...
├── skills/                     # 37개 스킬 (SKILL.md + templates/ + examples/)
